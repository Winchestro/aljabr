...
export default class Mesh {
	...
	createElement ( name, data, type, usage, material ) {
		let element = new ElementAttribute( name, data, material, type );
	...
	static Cube ( name = "cube" ) {
		let geometry = Geometry.Cube( name );
		let vertex = new VertexPtr( geometry );
		let { triangles, lines, points } = Mesh.Cube.options;
...
export class Face {
	constructor ( vPtrA, vPtrB, vPtrC ) {
		let hA = new Halfedge( vPtrA, this );
		let hB = new Halfedge( vPtrB, this );
		let hC = new Halfedge( vPtrC, this );
	...
	setEdges( oppositeHalfedgeA, oppositeHalfedgeB, oppositeHalfedgeC ) {
...
Getters( Face.prototype, {
	hEdgeB ( ) { return this.hEdgeA.next; },
	hEdgeC ( ) { return this.hEdgeA.prev; }

} );
export class Halfedge {
	constructor ( toVertex, belongsToFace ) {
	...
	setNeighbors ( previousHalfedge, nextHalfedge ) {
