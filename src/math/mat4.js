define( [
    ...
    class mat4 extends Float32Array {
        ...
        invert ( inM4A ) {
            ...
            else inM4A = CACHE_MAT4.transpose( inM4A );
            let a = inM4A;
        ...
        static lookAt ( outM4, inV3_eye, inV3_target, inV3_up ) {
            return mat4.prototype.lookAt.call( outM4, xPos, yPos, zPos );
        }
        ...
        rotate ( rad, x, y, z ) {
            ...
            if ( abs( length ) < EPSILON ) return mat4.makeIdentity( this );
        ...
        makeRotationQuat4 ( quat ) {
            ...
            let xx = quat[_x_] * two[_x_]; let xy = quat[_x_] * two[_y_]; let xz = quat[_x_] * two[_z_];
            let yy = quat[_y_] * two[_y_]; let yz = quat[_y_] * two[_z_]; let zz = quat[_z_] * two[_z_];
            let wx = quat[_w_] * two[_x_]; let wy = quat[_w_] * two[_y_]; let wz = quat[_w_] * two[_z_];
